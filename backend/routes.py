...
@jwt_required()
def page_find_all_for_property_set():
    session = Session()
    ...
    pre_answers = []
    for i in questions:
        [pre_answers.append(j) for j in session.query(PreAnswer).filter(PreAnswer.question_id == i.id)]
...
@application.route('/propertySetByEmployee/', methods=['GET'])
@jwt_required()
def find_property_set_by_employee():
    session = Session()
    ...
    if not (check_personal(session, get_jwt_identity()) == int(employee_id)):
        ...
        if check_owner(session, get_jwt_identity()):
            ...
            if 'team_id' in args:
                ...
                session.commit()
                session.close()
                res += 'Team changed to {}'.format(args['team_id'])
            return res, 200
...
def new_company():
    ...
    try:
        ...
        session.query(Employee).filter(Employee.email == owner_email).update(
            {'is_owner': True, 'company_id': company.id})
        session.commit()
...
def delete_company():
    ...
    session.query(Employee).filter(Employee.company_id == company_id).update(
        {'company_id': 1, 'role': False, 'is_owner': False})
...
def find_pre_answer_by_property_set_id():
    ...
    pre_answers = session.query(PreAnswer).filter(PreAnswer.question_id == question_id)
    pre_answer_schema = PreAnswerSchema()
...
def page_feedback_history():
    ...
    if not (check_admin(session, get_jwt_identity()) or check_personal(session, get_jwt_identity()) == int(
            employee_id)):
        return "Access denied", 403
